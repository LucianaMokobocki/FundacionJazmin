...
const projects = [
  ["Plaza Ituzaingó", "Montevideo", "/plazaituzaingo", images.ituzaingo],
  ["Jardín Botánico", "Montevideo", "/plazajardinbotanico", images.botanico],
  ["Plaza Portugal", "Montevideo", "/plazaportugal", images.portugal],
  ["Plaza Vázquez Ledesma", "Montevideo", "/vazquezledesma", images.vazquez],
] as const;
...
export default function Home() {
  return <main id="contenido">
    ...
    <section id="mision" className="section intro-section home-intro"><div className="container intro-layout"><p className="eyebrow">Nuestra razón de ser</p><h2>Jugar juntos puede transformar la sociedad.</h2><figure className="intro-figure intro-figure-main"><img src={images.event} alt="Familias reunidas en una actividad de Fundación Jazmín"/></figure><div className="intro-side"><p className="large-text">Fundación Jazmín nació el 16 de junio de 2015 con el objetivo de promover la inclusión de todos los niños en el espacio público, considerando sus diferentes capacidades, para contribuir desde esta experiencia a una transformación de la sociedad toda.</p><Link className="text-link" href="/sobrenosotros">Conocé nuestra historia <span aria-hidden="true">→</span></Link><figure className="intro-figure intro-figure-side"><img src={images.children} alt="Niños disfrutando de un espacio de juego inclusivo"/></figure></div><p className="intro-kicker" aria-hidden="true">Jugar · compartir · incluir</p></div></section>

    <section className="photo-statement"><div className="statement-layout"><figure className="statement-photo"><img src={images.family} alt="Jazmín junto a su familia"/></figure><div className="statement-card"><span>Más del</span><strong><span className="stat-value">6</span><span className="stat-percent">%</span></strong><p>de la población tiene algún tipo de discapacidad en Uruguay.</p></div></div></section>
    ...
    <section className="section projects-section"><div className="container"><div className="title-row"><SectionTitle eyebrow="Nuestro trabajo" title="Espacios donde nadie queda afuera."/><Link className="text-link" href="/proyectos">Ver todos los proyectos →</Link></div><div className="project-cards">{projects.map(([title,city,href,image],i)=><Link href={href} className={`project-feature feature-${i+1}`} key={title}><img src={image} alt=""/><div><span>0{i+1} · Proyecto</span><h3>{title}</h3><p>{city}</p></div></Link>)}</div></div></section>
